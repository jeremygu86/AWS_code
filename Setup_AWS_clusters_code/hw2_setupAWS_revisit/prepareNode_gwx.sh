...
# ubunut? doesn't matter. By default, they're not installed.
#disable SELinux
echo -e "\nDisabling SELinux"
sudo sed -i 's/SELINUX=enforcing/SELINUX=disabled/g' /etc/selinux/config

# Turn off autostart of iptables and ip6tables
echo -e "\nChecking ipTables and ip6table are off" ## ubuntu sudo ufw disable
...
sudo mkdir /var/log /data0 /data1 /data3
...
## No instant storage 2015/11/14
# sudo sed -i '$ a/dev/xvdc /var/log ext4 defaults 0 0' 	/etc/fstab
sudo sed -i '$ a/dev/xvdc /data2 ext4 defaults,noatime 0 0' /etc/fstab
...
sudo mount –o remount /data1, /data2, /data0
