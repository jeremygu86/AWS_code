...
# snappy-devel
echo -e "Remove snappy"
sudo yum remove snappy
echo -e "Install snappy"
sudo yum -y install ln -sf /usr/lib64/libsnappy.so /usr/lib/hadoop/lib/native/.
...
sudo sed -i '$ a/dev/xvdf /data0 ext4 defaults 0 0' /etc/fstab
sudo sed -i '$ a/dev/xvdg /data1 ext4 defaults 0 0' /etc/fstab
